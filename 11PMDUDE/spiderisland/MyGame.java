...
public class MyGame extends VariableFrameRateGame
{
	...
	private boolean paused=false, punching = false, onDolphin=true, colliding=false, visitedObj1=false, visitedObj2=false, visitedObj3=false, visitedObj4=false, gameOver=false, allVisited=false, touchingLava=false, axisLines=true, visitLock1=false, visitLock2=false, visitLock3=false, visitLock4=false, isClientConnected=false;	private int counter=0, score=0, holder=0;
...
private GameObject objCube, objSphere, objTorus, objPlane, objDiamond, objLava, objChest, objXLine, objYLine, objZLine, objGhost, objSpider, objPlayerPunchBox;	//Refridgerator magnets
	...
	{	Matrix4f initialTranslation, initialScale, initialRotation, rotateX, rotateY, rotateZ;
		...
		initialTranslation = (new Matrix4f()).translation(getSpawnCoord(), -20, getSpawnCoord()); //Create the initial transformation matrices (in this sense its the initial translation and the initial scaling of the cube)
		...
		initialTranslation = (new Matrix4f()).translation(2, -20.5f, 2);
		...
		initialTranslation = (new Matrix4f()).translation(2, -10, 2);
		...
		initialTranslation = (new Matrix4f()).translation(getSpawnCoord(), -20.25f, getSpawnCoord());
		...
		initialTranslation = (new Matrix4f()).translation(getSpawnCoord(), -30f, getSpawnCoord());
		initialScale = (new Matrix4f()).scaling(3f);
		...
		initialTranslation = (new Matrix4f()).translation(getSpawnCoord()+ 1000f, -10.1f, getSpawnCoord());
		...
		{	
			...
			case KeyEvent.VK_R:
				if(choosingModel){
					...
					(engine.getSceneGraph()).removeGameObject(objPlayerPunchBox);
